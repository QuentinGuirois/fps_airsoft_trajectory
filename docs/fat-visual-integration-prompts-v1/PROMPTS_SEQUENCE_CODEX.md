...
## Prompt 1 — Audit, tokens, polices, palette sombre, logo et loader
...
4. un thème sombre unique, appliqué dans le `<head>` avant le premier paint, sans sélecteur ni préférence locale ;
5. mise à jour de `color-scheme`, `theme-color`, des SVG inline et des couleurs Canvas déjà utilisées ; le Canvas doit lire les variables via `getComputedStyle` ;
...
Ajoute des tests pour le thème sombre, le loader, les actifs et l’absence de CDN. Incrémente la version du cache. Lance tous les tests et vérifie en HTTP, au clavier et avec reduced motion.

Arrête-toi après ce lot. Rends : fichiers modifiés, tests, captures sombres du header et du loader, preuve que le moteur et les données sont inchangés, problèmes éventuels. Ne fais pas de commit.
...
- toutes les couleurs Canvas viennent des variables CSS de la palette sombre au moment de dessiner ;
...
Arrête-toi après la 2D. Rends les captures sombres desktop/mobile, les tests et la preuve que les sorties numériques ATP sont inchangées. Ne fais pas de commit.
...
Gère le cycle de vie : redimensionnement, pause hors écran, annulation de l’animation et destruction des ressources et listeners. Avec reduced motion, aucune lecture automatique et caméras instantanées.
...
5. thème sombre unique, absence de flash et palette cohérente dans Canvas/3D ;
...
Produis des captures sombres desktop/mobile et un rapport final avec fichiers modifiés, tests exécutés, résultats, poids initial/3D, différences connues et éventuels contenus manquants. Ne masque aucun échec. Ne déclare terminé que lorsque tout est vert. Ne fais pas de commit.
